...
import { CartButton } from "@/components/header-with-search/cart-button";
import { CartButtonWithDropdown } from "@/components/header-with-search/cart-button-with-dropdown";
import { useCart } from "@/components/cart";

const SHOP_HEADER_BG = "rgb(208,1,27)";

interface ShopMainHeaderProps {
  shopSlug: string;
}

export function ShopMainHeader({ shopSlug }: ShopMainHeaderProps) {
  const { itemCount } = useCart();

  return (
    <div
      className="border-b border-black/10 z-[300]"
      style={{ backgroundColor: SHOP_HEADER_BG }}
    >
      <div className="mx-auto flex w-full max-w-[1200px] flex-col gap-3 px-3 py-3 sm:px-4 lg:flex-row lg:items-center lg:gap-0 lg:px-0 lg:py-3.5 lg:pb-5">
        <div className="flex items-center justify-between gap-3 lg:mr-[15px] lg:w-auto lg:justify-start">
          <div className="flex min-w-0 items-center">
            <Link
              href="/"
              className="rounded-[2px] -m-0.5 p-0.5 outline-none no-underline cursor-pointer focus-visible:ring-2 focus-visible:ring-blue-600"
              aria-label="Shopee Home"
            >
              <svg
                enableBackground="new 0 0 48 48"
                viewBox="0 0 48 48"
                className="block h-10 w-9 fill-white relative overflow-hidden cursor-pointer lg:h-[45px] lg:w-[2.5rem]"
                aria-hidden
              >
            ...
            <div
              className="mx-3 h-10 w-px shrink-0 self-center bg-white/40 lg:mx-4 lg:h-[45px]"
              aria-hidden
              role="presentation"
            />
            <Link
              href="/mall"
              className="rounded-[2px] outline-none no-underline cursor-pointer flex items-center transition-transform transition-opacity duration-300 focus-visible:ring-2 focus-visible:ring-white"
            >
              <svg
                viewBox="0 0 220 44"
                className="relative block h-7 w-auto max-w-[140px] overflow-hidden fill-white lg:top-[5px] lg:h-[33px] lg:max-w-[165px]"
                aria-hidden
              >
                <g fill="#fff" fillRule="evenodd" transform="translate(0 -.098)">
                  ...
                  <path d="M171.002 3.338c-1.545 4.329-5.841 7.27-10.7 7.186-4.8-.084-8.954-3.103-10.394-7.412-.33-.983-1.407-1.519-2.408-1.197a1.865 1.865 0 0 0-1.217 2.364c1.946 5.823 7.526 9.879 13.954 9.991 6.505.114 12.277-3.84 14.365-9.69a1.866 1.866 0 0 0-1.17-2.39c-.993-.342-2.082.171-2.43 1.148z" fillRule="nonzero" />
        ...
        <div className="hidden min-w-0 shrink flex-1 lg:block" />

        <div className="flex w-full items-center lg:w-[36.875rem] lg:shrink-0">
          <ShopSearchBar shopSlug={shopSlug} />
        </div>

        <div className="hidden shrink-0 items-center justify-end lg:flex lg:flex-1">
