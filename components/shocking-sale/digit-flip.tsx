...
const digitPaths: Record<number, { path: string; viewBox: string }> = {
  ...
  6: {
    path: "M24.5 53.2v13.7h12.3V53.2h6.1v-11h-6.1V0H23.4L0 42.5v10.7h24.5zm0-11h-14l14-24.8v24.8z",
    viewBox: "0 0 42.9 66.9",
  },
  7: {
    path: "M22.5 11L7.3 31.3l7.2 6.1c1.9-1.2 3.8-1.7 6.1-1.7a10.732 10.732 0 0 1 5.741 1.573c2.816 1.762 4.559 4.87 4.559 8.627a10.175 10.175 0 0 1-2.626 6.924A10.165 10.165 0 0 1 20.7 56.1c-4.086 0-7.199-2.156-8.584-5.887A12.3 12.3 0 0 1 11.5 47.8L0 48.2c.595 4.955 1.386 7.553 3.347 10.52a24.795 24.795 0 0 0 .053.08 19.859 19.859 0 0 0 13.535 8.466A24.541 24.541 0 0 0 21 67.6c11.745 0 20.628-8.083 22.012-19.459A25.999 25.999 0 0 0 43.2 45a22.484 22.484 0 0 0-2.049-9.694c-2.825-5.949-8.41-9.794-15.751-10.406L43.9 0H3.3v11h19.2z",
    viewBox: "0 0 43 67",
  },
  8: {
    path: "M21.6 56.403l12.5-18.3c5.081-7.491 7.492-12.684 8.063-17.877a22.986 22.986 0 0 0 .137-2.523A17.062 17.062 0 0 0 30.133 1.242 24.304 24.304 0 0 0 22.3.003a23.551 23.551 0 0 0-9.787 1.948C5.323 5.212 1.199 12.42 1.199 22.443a36.776 36.776 0 0 0 .001.26H13a70.067 70.067 0 0 1 .052-1.368c.118-2.398.351-3.497.948-5.032a7.181 7.181 0 0 1 5.187-4.431 9.74 9.74 0 0 1 2.313-.269c4.599 0 7.899 3.099 7.9 7.398a5.467 5.467 0 0 1 0 .002 11.162 11.162 0 0 1-.092 1.496c-.29 2.167-1.352 4.335-4.331 9.011a261.731 261.731 0 0 1-1.677 2.593L0 67.703h40.5v-11.3H21.6z",
    viewBox: "0 0 42.3 67.703",
  },
  9: {
    path: "M12.1 18.4v49.7H25V0L0 13v11.9l12.1-6.5z",
    viewBox: "0 0 25 68.1",
  },
};

export function DigitFlip({ value, animationName, duration }: DigitFlipProps) {
  const prevValueRef = useRef(value);
  const [key, setKey] = useState(0);

  useEffect(() => {
    if (prevValueRef.current !== value) {
      // Value changed, trigger animation by updating key
      setKey((prev) => prev + 1);
      prevValueRef.current = value;
    }
  }, [value]);

  // Calculate the translateY position based on current value
  // Each digit is 17px apart (h-4 = 16px + 1px spacing)
  const translateY = -value * 17;

  return (
    <div
      key={key}
      className="w-2.5 h-48 inline-block overflow-x-hidden overflow-y-hidden relative"
      style={{
        transform: `translateY(${translateY}px)`,
        transition: "transform 0.3s cubic-bezier(0.4, 0, 0.2, 1)",
      }}
    >
      {/* Render all digits 0-9 stacked vertically, each 17px apart */}
      {Array.from({ length: 10 }, (_, i) => (
        <div
          key={i}
          className="w-2.5 h-4 relative"
          style={{
            position: "absolute",
            top: `${i * 17}px`,
            left: "0",
        ...
        >
          <svg
            viewBox={digitPaths[i].viewBox}
            role="img"
            className="align-baseline inline fill-white stroke-white w-4 h-3.5 absolute overflow-x-visible overflow-y-visible -translate-x-2/4 -translate-y-2/4 left-2/4 top-2/4"
          >
            <path
              fillRule="evenodd"
              strokeLinecap="round"
