...
export type SearchSortOption = "relevance" | "latest" | "top_sales" | "price_asc" | "price_desc";
...
export function SearchFilterSortBar({
  ...
  useEffect(() => {
    const found = PRICE_OPTIONS.find(
      (o) => o.value && (sortOption === "price_asc" || sortOption === "price_desc") && o.value === sortOption
    );
  ...
  const baseSort = sortOption === "price_asc" || sortOption === "price_desc" ? "relevance" : sortOption;
    ...
    >
          ...
          {SORT_OPTIONS.map((opt) => (
            <button
              ...
              className={`h-[34px] leading-[34px] rounded-sm shadow-[0_1px_1px_rgba(0,0,0,0.02)] cursor-pointer select-none capitalize whitespace-nowrap box-border justify-center items-center min-w-[90px] px-4 flex relative outline-0 overflow-visible appearance-none text-sm font-normal ${
                baseSort === opt.value
                  ? "bg-[rgb(238,77,45)] text-white border-0"
        ...
        <section className="block" ref={dropdownRef}>
          <div className="relative">
            ...
            {priceOpen && (
              <div
                role="listbox"
                className="absolute left-0 top-full mt-1 z-50 bg-white rounded-sm shadow-lg border border-black/9 min-w-[200px] py-1"
              >
                {PRICE_OPTIONS.filter((o) => o.value).map((opt) => (
                  <button
                    ...
                    onClick={() => {
                      onSortChange(opt.value as SearchSortOption);
                      setPriceOpen(false);
                    }}
                    className={`w-full text-left px-3 py-2 text-sm hover:bg-black/5 focus:bg-black/5 focus:outline-none ${
                      sortOption === opt.value ? "bg-red-50 text-[rgb(238,77,45)]" : ""
                    }`}
                  >
                    {opt.label}
                  </button>
      ...
      <div className="flex items-center">
        ...
        {currentPage <= 1 ? (
          <span
            aria-disabled
            className="ml-5 rounded-l-sm rounded-r-none w-9 h-[34px] shadow-[0_1px_1px_rgba(0,0,0,0.05)] pointer-events-none flex justify-center items-center text-sm font-light text-black/26 bg-[rgb(249,249,249)] border border-r border-black/[0.09]"
          >
          ...
          <Link
            ...
            className="ml-5 rounded-l-sm rounded-r-none w-9 h-[34px] shadow-[0_1px_1px_rgba(0,0,0,0.05)] flex justify-center items-center text-sm font-light text-black/80 bg-[rgb(249,249,249)] border border-r border-black/[0.09] no-underline hover:bg-black/5 transition-colors"
          >
