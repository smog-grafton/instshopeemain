...
export default function UserAccountPhonePage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        setCurrentPhone(profile.phoneNumber ?? null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessage({ type: "success", text: trimmed ? "Phone number updated." : "Phone number removed." });
      setCurrentPhone(trimmed);
      if (!trimmed) setPhone("");
      setTimeout(() => setMessage(null), 3000);
    } catch (err: unknown) {
      const text = err instanceof Error ? err.message : "Failed to update phone number. Please try again.";
